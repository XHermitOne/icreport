    ...
    try:
        ...
        for section in sections:
            ...
            for param in params:
                ...
                dictionary[section][param] = param_value
...
def Dict2INI(dictionary, ini_filename, bRewrite=False):
    ...
    try:
        if not dictionary:
            log.warning(u'Не определен словарь <%s> для сохранения в INI файле' % dictionary)
            return False
        ...
        for section in dictionary.keys():
            ...
            for param in dictionary[section].keys():
                ini_parser.set(section_str, str(param), str(dictionary[section][param]))
